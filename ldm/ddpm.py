...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cond3_order = list(cond_stage_config.cond3_order)
    ...
    def instantiate_cond_stage(self, config):
        if config.cond_type=="pretrained_vit":
            ...
            print("Only fc layers are trainable, other layers are frozen")
    ...
    def get_input(
        ...
        if self.high_low_mode:
            ...
            cond_cat = []
            if 1 in self.cond_nums:
                cond1 = self.condition_vit_encode(cond1).permute(self.cond1_order)
                cond_cat.append(cond1)
            if 2 in self.cond_nums:
                cond2 = self.condition_vit_encode(cond2).permute(self.cond2_order)
                cond_cat.append(cond2)
            if 3 in self.cond_nums:
                cond3 = self.condition_vit_encode(cond3).permute(self.cond3_order)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 1:
            ...
            gt = gt / 255 * 2500
            rec = rec / 255 * 2500
            _, _, _, ssim_avg = Structural_Similarity(gt, rec, PIXEL_MAX=4095)
